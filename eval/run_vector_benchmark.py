...
class VectorBenchmarkRunner:
    ...
    def run_single_qa(self, qa: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Vector RAG 쿼리 실행
            result = self.vector_engine.process_query(question)
            
            # 결과 추출
            predicted_answer = result.get('answer', '')
            confidence = result.get('confidence', 0.0)
            
            # 평가 메트릭 계산 (Vector RAG 답변 형식에 맞게 조정)
            metrics = self._calculate_vector_metrics(predicted_answer, ground_truth)
    ...
    def _calculate_vector_metrics(self, predicted_answer: str, ground_truth: Dict[str, Any]) -> Dict[str, float]:
        """Vector RAG 답변에 대한 메트릭 계산"""
    ...
    def _generate_report(self):
        ...
        # JSON 보고서
        json_path = self.eval_dir / "vector_benchmark_report.json"
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(self.results, f, ensure_ascii=False, indent=2)
        print(f"  ✅ JSON 보고서: {json_path}")
        
        # Markdown 보고서
        md_path = self.eval_dir / "VECTOR_BENCHMARK_REPORT.md"
    ...
    def _generate_markdown_report(self, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write("# Vector RAG 벤치마크 실행 보고서\n\n")
...
def main():
    ...
    runner = VectorBenchmarkRunner(base_dir)
